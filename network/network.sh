...
function explorerUp() {
  ...
  if [ ! -d "explorer/connection-profile" ]; then
    ...
    IPADDRESS=$(docker inspect orderer.example.com | awk 'NR == 258 {print $2}' | tr -d '",')
    sed -i "s/ORDERER_IP/${IPADDRESS}/g" ./explorer/connection-profile/hyperledger-explorer.json
    IPADDRESS=$(docker inspect peer0.org1.example.com | awk 'NR == 278 {print $2}' | tr -d '",')
    sed -i "s/PEER0ORG1_IP/${IPADDRESS}/g" ./explorer/connection-profile/hyperledger-explorer.json
    IPADDRESS=$(docker inspect ca_peerOrg1 | awk 'NR == 227 {print $2}' | tr -d '",')
