...
function generateCerts() {
  # 실행 프로그램 찾는 명령어인 which => 실제로 이동해보면 해당 파일이 있는것을 확인
  which cryptogen
  if [ "$?" -ne 0 ]; then
...
function explorerUp() {
  ...
  if [ ! -d "explorer/connection-profile" ]; then
    ...
    IPADDRESS=$(docker inspect orderer.example.com | awk 'NR == 278 {print $2}' | tr -d '",')
    sed -i "s/ORDERER_IP/${IPADDRESS}/g" ./explorer/connection-profile/hyperledger-explorer.json
    IPADDRESS=$(docker inspect peer0.org1.example.com | awk 'NR == 288 {print $2}' | tr -d '",')
    sed -i "s/PEER0ORG1_IP/${IPADDRESS}/g" ./explorer/connection-profile/hyperledger-explorer.json
    IPADDRESS=$(docker inspect ca_peerOrg1 | awk 'NR == 237 {print $2}' | tr -d '",')
